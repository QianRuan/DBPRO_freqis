...
public class AprioriFlink {
	...
	public AprioriFlink(DataSet<List<Integer>> transactionList, int minSupport,
                        int numIterations) {
	...
	private static DataSet<String> getTextDataSet(ExecutionEnvironment env, String dataPath) {
		...
		if(dataPath != null) {
			// read the csv file from resources path
			return env.readTextFile(dataPath);
		} else {
			// get default  csv data
